...
class SingleLinkedList
{
	...
	{
		Node() = default;
		Node(const Type& val, Node* next_) : value(val), next(next_) {}
    ...
    public:
        ...
        BasicIterator(const BasicIterator<Type>& rhs) noexcept
        {
            node_ = rhs.node_;
        }
        ...
        _NODISCARD bool operator==(const BasicIterator<const Type>& rhs) const noexcept
        {
        ...
        _NODISCARD bool operator==(const BasicIterator<Type>& rhs) const noexcept
        {
            return node_ == rhs.node_;
        }
...
public:
    ...
    _NODISCARD ConstIterator cbegin() const noexcept
    {
        return ConstIterator{ head_.next };
    }

    _NODISCARD ConstIterator cend() const noexcept
    {
        return ConstIterator{ nullptr };
    }
    ...
    _NODISCARD ConstIterator cbefore_begin() const noexcept
    {
    ...
    SingleLinkedList(const SingleLinkedList& other)
    {
        ...
        SingleLinkedList tmp2{ tmp.begin(), tmp.end() };
        swap(tmp2);
    }

    SingleLinkedList(std::initializer_list<Type> list)
    {
        SingleLinkedList tmp{ list.begin(), list.end() };
        SingleLinkedList tmp2{ tmp.begin(), tmp.end() };
        swap(tmp2);
    }
	...
	{
		Node* tmp = new Node(val, head_.next);
        head_.next = tmp;
		++size_;
	}
    ...
    {
        if (!pos.node_->next) return Iterator{ nullptr };
        Node* tmp = pos.node_->next->next;
    ...
    {
        Node* tmp = new Node(value, pos.node_->next);
        pos.node_->next = tmp;
        ++size_;
        return Iterator{tmp};
    }
